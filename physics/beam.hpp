...
inline Vec3 beamForce(Beam &beam, const Node &a, const Node &b, bool canBreak = true) {
    ...
    if (length < 1e-7f)
        return {}; // Explicit zero-length guard, absent in upstream approximation.
    ...
    if (magnitude > threshold) {
        if (beam.k != 0) {
            if (force > beam.compressionYield && extension < 0) {
                ...
                magnitude = force;
                if (oldLength > beam.rest)
                    beam.compressionYield *= oldLength / beam.rest;
                ...
                magnitude = -force;
                if (oldLength > 0 && beam.rest > oldLength)
                    beam.tensionYield *= beam.rest / oldLength;
        ...
        if (magnitude > beam.strength) {
            if (canBreak) {
                ...
                beam.strength =
                    2.0f * std::min({beam.compressionYield, -beam.tensionYield, beam.strength});
        }
    }
